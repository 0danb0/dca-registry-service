...
import com.danb.dca.registry_service.models.response.CommonErrorResponse;
import com.danb.dca.registry_service.models.response.CustomError;
...
public class ErrorHandler implements RequestBodyAdvice {
    ...
    public final CommonErrorResponse handleAllExceptions(Exception ex, HttpServletRequest request) {
        ...
        return buildError(request, ErrorMsg.DCA_RGT_SRV_99.getCode(), ErrorMsg.DCA_RGT_SRV_99.getMessage(), ex.getMessage(), DomainMsg.MICROSERVICE_FUNCTIONAL.getName());
    }

    @ExceptionHandler(RegistryException.class)
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    public final CommonErrorResponse handleInvoiceExceptions(RegistryException ex, HttpServletRequest request) {
        log.error("Exception: ", ex);
        return buildError(request, ex.getCode(), ex.getMessage(), ex.getMessage(), DomainMsg.REGISTRY_SERVICE_TECHNICAL.getName());
    }

    @ExceptionHandler(MainException.class)
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    public final CommonErrorResponse handleMainExceptions(MainException ex, HttpServletRequest request, String code, String message) {
        log.error("Exception: ", ex);
        return buildError(request, code, message, ex.getMessage(), DomainMsg.REGISTRY_SERVICE_TECHNICAL.getName());
    }
...
//        log.error("MethodArgumentNotValidException: ", ex);
        ...
        if (ex instanceof MethodArgumentNotValidException methodArgumentNotValidException) {
            ...
            return buildError(request, ErrorMsg.DCA_RGT_SRV_01.getCode(), ErrorMsg.DCA_RGT_SRV_01.getMessage(), ex.getMessage(), DomainMsg.MICROSERVICE_FUNCTIONAL.getName());
        }
    ...
    public final CommonErrorResponse handleInvalidJson(HttpMessageNotReadableException ex, HttpServletRequest request) {
        ...
        return buildError(request,
                ErrorMsg.DCA_RGT_SRV_01.getCode(),
    ...
    private CommonErrorResponse buildInvalidFieldsError(HttpServletRequest request, Map<String, String> fieldsMap) {
        ...
        return buildError(request, ErrorMsg.DCA_RGT_SRV_01.getCode(), message, StringUtils.join(fieldsMap.values(), ','), DomainMsg.MICROSERVICE_FUNCTIONAL.getName());
    }
